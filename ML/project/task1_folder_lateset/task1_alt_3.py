...
results_dir = 'task1_3_results'   
os.makedirs(results_dir, exist_ok=True)
learning_rates_lists = [1.0, 1e-1, 1e-2, 1e-3, 1e-4]
models = {
...
for lr in learning_rates_lists:
    for model_name, model_class in models.items():
        ...
        sub_dir = os.path.join(results_dir, f'{model_name}_{lr}_lr')
        ...
        plt.title(f'Trajectory Tracking with and without {model_name}_{lr} Correction', fontsize=22)
        ...
        plt.title(f'Position Error with and without {model_name}_{lr} Correction', fontsize=22)
        plt.xlabel('Time [s]' , fontsize=18)
...
# training losses plosts for different learning rates setting
plt.figure(figsize=(12, 6))
for model_name in df_results['Model'].unique():
    subset = df_results[df_results['Model'] == model_name]
    plt.plot(subset['Learning Rate'], subset['Final Loss'], marker='o', label=model_name)

plt.xscale('log')
plt.xlabel('Learning Rate (log scale)', fontsize=18 )
plt.ylabel('Final Loss', fontsize=18)
plt.title('Final Loss vs. Learning Rate for Shallow and Deep MLPs',fontsize=22)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.legend(fontsize=16)
plt.savefig(os.path.join(results_dir, 'compare.png'))
plt.show()
